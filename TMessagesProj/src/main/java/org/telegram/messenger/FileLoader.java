...
//            FileLog.d("load operation update position fileName=" + finalFileName + " position in queue " + operation.getPositionInQueue() + " preloadFinish " + operation.isPreloadFinished() + " priority=" + operation.getPriority());
    ...
    /**
     * Forces all of this account's file-load queues to immediately
     * re-evaluate how many concurrent downloads are allowed. Called when a
     * voice/video call starts or ends, so background downloads throttle
     * down (or resume) right away instead of waiting for some unrelated
     * queue event to trigger the next check.
     */
    public void recheckQueuesForCallStateChange() {
        for (int i = 0; i < smallFilesQueue.length; i++) {
            smallFilesQueue[i].checkLoadingOperations(true);
            largeFilesQueue[i].checkLoadingOperations(true);
        }
    }

    public static void recheckQueuesForCallStateChangeAllAccounts() {
        for (int a = 0; a < UserConfig.MAX_ACCOUNT_COUNT; a++) {
            if (Instance[a] != null) {
                Instance[a].recheckQueuesForCallStateChange();
            }
        }
    }
