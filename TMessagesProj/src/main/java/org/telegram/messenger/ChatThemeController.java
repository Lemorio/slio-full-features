...
public class ChatThemeController extends BaseController {
    ...
    private void init() {
        ...
        // Cloud chat theme catalog removed - don't repopulate allChatThemes
        // from local cache, always start empty.
        allChatThemes = new ArrayList<>();
    ...
    public void requestAllChatThemes(final ResultCallback<List<EmojiThemes>> callback, boolean withDefault) {
        // Cloud chat theme catalog fetching removed along with the theme
        // browser/editor - always report an empty list immediately, no
        // network request. Per-chat wallpapers (unrelated feature) still
        // work normally.
        if (allChatThemes == null) {
            allChatThemes = new ArrayList<>();
        }
        callback.onComplete(getEmojiThemes(withDefault ? THEME_LIST_WITH_DEFAULT : 0));
    }
    ...
    public boolean isAllThemesFullyLoaded() {
        // Cloud chat theme catalog removed - allChatThemes is intentionally
        // always empty (initialized, never "still loading").
        return isGiftThemesFullyLoaded() && allChatThemes != null;
    }
    ...
    private void requestNextChatThemes(ResultCallback<Void> callback) {
        // Unique-gift chat theme catalog fetching removed along with the
        // theme browser/editor - report "complete, nothing more to load"
        // immediately, no network request.
        if (giftsThemeList.themes == null) {
            giftsThemeList.themes = new ArrayList<>();
        }
