...
public class QRScanner {
    ...
    private Detected detect(Bitmap bitmap) {
        ...
        final QRCodeReader detector = this.detector.get();
        if (detector == null) {
            return null;
        }
        ...
        final RGBLuminanceSource source = new RGBLuminanceSource(w, h, intArray);

        final Result result;
        try {
            result = detector.decode(new BinaryBitmap(new GlobalHistogramBinarizer(source)));
        } catch (Exception e) {
            return null;
        }
        ...
        link = link.trim();
        if (!link.startsWith(prefix) && !link.startsWith("https://" + prefix) && !link.startsWith("http://" + prefix)) return null;

        final ResultPoint[] resultPoints = result.getResultPoints();
        if (resultPoints == null || resultPoints.length == 0) return null;
