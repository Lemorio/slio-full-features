...
public class GroupCallPipAlertView extends LinearLayout implements VoIPService.StateListener, NotificationCenter.NotificationCenterDelegate {
    ...
    public GroupCallPipAlertView(Context context, int account) {
        ...
        closeButton.setColorFilter(new android.graphics.PorterDuffColorFilter(ColorUtils.setAlphaComponent(Color.WHITE, (int) (255 * 0.8f)), android.graphics.PorterDuff.Mode.SRC_IN));
        closeButton.setBackground(Theme.createSelectorDrawable(ColorUtils.setAlphaComponent(Color.WHITE, (int) (255 * 0.2f)), Theme.RIPPLE_MASK_CIRCLE_20DP));
        closeButton.setOnClickListener(v -> {
            if (onCloseClickListener != null) {
                onCloseClickListener.run();
            }
        });
        groupInfoContainer.addView(closeButton, LayoutHelper.createFrame(28, 28, Gravity.TOP | Gravity.RIGHT, 0, 4, 4, 0));
